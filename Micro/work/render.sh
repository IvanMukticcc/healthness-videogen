...
BED="${BED:-../../engine/sfx/flow_soft_8s.m4a}"
# 0.62 is Exercise's, and the two variants now share the synthesis as well as
# the gain: measured, the finished cut lands on -18.0 LUFS either way, so this is
# about the two clips being one sound rather than about level.
POP_GAIN="${POP_GAIN:-0.62}"
...
ffmpeg -y -loglevel error \
    -i "${TOPIC}_silent.mp4" -i "$BED" -i "${TOPIC}_pops.wav" \

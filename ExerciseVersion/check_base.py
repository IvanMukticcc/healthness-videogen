...
def main():
    ...
    for k, i in enumerate(order, 1):
        band = lab == i
        ys, xs = np.where(band)
        # Only the middle stretch: the figure and the body are meant to cover the
    ...
    if bad:
        ...
        # at the poster: the labels belong under the figure and under the body.
...
def check_labels(new, base, mask, anchor_l=330, anchor_r_x=1190):
    ...
    The generator sometimes puts the lift's label above the figure instead of under
    ...
    # Type only. A lifter's white singlet is as bright as a label, and the title is
    # brighter still; both were being read as the row's caption. A letter stroke
    # cannot contain a 21x21 square, a torso can.
    ...
    for k, i in enumerate(order, 1):
        ...
        for name, cx in (("lift", anchor_l), ("body", anchor_r_x)):
            ...
            # A caption is wide and short. A dumbbell plate and stray highlights

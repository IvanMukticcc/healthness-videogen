...
## Where things live
    ...
    ShortPrompt/
      engine/          the five tools, the three authored assets, sfx/. Read its README
      Foods/           this variant
        INPUT/         the base image, and nothing else: what gets attached to the prompt
        OUTPUT/        the finished clips, a folder per day
        work/          the bases, the labelled posters, the prompts, this file
      Micro/ Exercise/ the other two variants, same three folders each

Run everything from inside `Foods/work/`, calling `../../../engine/<tool>.py`.
**Never keep a copy of an engine file here** - that is how the same five tools
ended up at three different generations at once. `../../engine-status.sh`
reports what each variant is actually running.
...
| `../../engine/sfx/` | the water beds, 8 s, normalised to -18 LUFS |
...
../.venv/bin/python ../../engine/flowanim.py <poster>_labelled.png --width 1080 --seconds 8 \
    ...
    --anchored ../INPUT/base_<topic>.png --layout base_<topic>_layout.json \
...
```
ffmpeg -y -i /tmp/<topic>.mp4 -i ../../engine/sfx/flow_soft_8s.m4a \

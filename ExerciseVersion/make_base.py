...
def taper_tail(L, length, fade):
    """Thin the wave to a point at the body end, and dissolve the last of it.

    A blunt tail has to be hidden by whatever sits in the right circle, and in
    the food version that was a generated organ whose size and height varied, so
    some of it always showed above or below. Here the body map is authored and
    always the same size - but the taper is kept, because it is also what makes
    the liquid read as arriving rather than as stopping. Trimming the wave
    instead risks a gap when whatever fills the circle lands further
...
def main():
    ...
    # Pull the wave in from the edges. The figure and the body are placed against
    # its two tips, so a wave that runs to the edge of the poster drags them out
    # there with it - and a vertical feed crops the sides, taking the body and
    ...
    if args.level:
        ...
        # met the body at the top of its circle rather than in the middle, and
    ...
    # under the last wave while every row was still too short to hold a figure and

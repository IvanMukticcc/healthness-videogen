...
def fit_all(texts, target_w, start=90):
    """One size for all ten captions - the widest one sets it.

    Fitting each caption on its own gives every bar a different size: 'OATS' came
    out at 60px next to 'SLOWER ABSORPTION' at 38, and a poster of ten sizes reads
    as ten decisions rather than one label style. So the size is found once, for
    the longest string, and every caption is drawn at it. The price is that one
    long caption shrinks all ten - which is the reason to keep them short.
    """
    start = max(20, start)
    size = start
    while size > 14:
        f = ImageFont.truetype(FONT, size)
        if all(f.getbbox(t)[2] - f.getbbox(t)[0] <= target_w for t in texts):
            return f
        size -= 2
...
def main():
    ...
    texts = [t.strip() for pair in pairs for t in pair]
    cap_h = min(r["cap_h"] for r in L["rows"])
    font = fit_all(texts, int(args.caption_w * 0.88), int(cap_h * 0.78))
    widest = max(texts, key=lambda t: font.getbbox(t)[2] - font.getbbox(t)[0])
    print(f"  every caption at {font.size}px, the size '{widest}' fits in")

...
def main():
    ...
    # The muscle badges and the body they light. Off unless --muscles is given,
    # so a command line that only wants the liquid still renders exactly what it
    # used to.
    ...
    p.add_argument("--muscle-d", type=float, default=210.0,
    ...
    p.add_argument("--muscle-dir", default=muscle_overlay.MUSCLE_DIR)
    p.add_argument("--muscle-cues", help="write the landing times here, for the SFX")
    # The body in the right circle. It is drawn here rather than baked into the
    # poster because a muscle has to light on the frame its badge lands, and a
    # baked body cannot do anything on a frame.
    p.add_argument("--body", type=int, default=1,
                   help="0 leaves the right circle empty, for a poster that fills it "
                        "itself")
    p.add_argument("--body-scale", type=float, default=2.20,
                   help="figure height as a multiple of the guide circle's radius; "
                        "over 2.0 the head and feet run past the disc, which is what "
                        "stops it reading as a sticker inside a button")
    ...
    if args.anchored:
        ...
        left = (np.abs(arr - anc).max(axis=2) < args.anchor_tol) & (np.abs(anc - cb).max(axis=2) > 3)
        ...
        left &= np.abs(arr - anc).max(axis=2) < args.anchor_tol
        arr[left] = cb[left]
    ...
    if args.muscles:
        if not args.layout:
            sys.exit("--muscles needs --layout: the badges sit on the row the "
                     "layout describes, and the body sits in its circle")
        rows = muscle_overlay.resolve(args.muscles)
        times = [float(t) for t in args.muscle_times.split(",")]
        curves = None
        if args.muscle_follow:
        ...
        mplan = muscle_overlay.plan(rows, args.layout, W, H, times, curves=curves,
                                    diameter=args.muscle_d, gap=args.muscle_gap,
                                    stagger=args.muscle_stagger, dy=args.muscle_dy,
                                    muscle_dir=args.muscle_dir, seconds=args.seconds,
                                    fade=args.muscle_fade)
        cues = muscle_overlay.pop_times(mplan)
        print(f"  {len(mplan)} muscle badges land at "
              f"{', '.join(f'{t:.2f}' for t in cues)}s")
        if args.body:
            # The same rows, the same times: the muscle lights on the frame its
            # badge lands. Passing the clean base lets each row's disc take that
            # row's own background colour instead of one grey for all five.
            bplan = body_overlay.plan(rows, args.layout, W, H, times,
                                      stagger=args.muscle_stagger, base=base,
                                      scale=args.body_scale, dy=args.muscle_dy,
                                      disc=bool(args.body_disc))
            print("  bodies: " + ", ".join(f"r{i + 1} {b['view']}"
                                           for i, b in enumerate(bplan)))
        if args.muscle_cues:
    ...
    for f in range(frames):
        ...
        # Last, and outside the protect pass: the body and the badges are drawn
        # on top of the finished frame, so nothing in the liquid pipeline has to
        # know they exist. The body first - it ends the wave where the organ used
        # to end it, and a badge is never behind it.
        if bplan:
            body_overlay.paint(out, bplan, f / args.fps)
        if mplan:
            muscle_overlay.paint(out, mplan, f / args.fps, ring=bool(args.muscle_ring))

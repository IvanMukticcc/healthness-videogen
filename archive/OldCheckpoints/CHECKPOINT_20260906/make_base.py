...
def main():
    ...
    p.add_argument("--span", default="0.20,0.79",
    ...
    edges = [e for i, e in enumerate(edges) if i == 0 or e - edges[i - 1] > 60 * args.ss]

    for k in range(len(edges) - 1):
        a, b = edges[k], edges[k + 1]
        band = np.zeros((H, W), bool); band[a:b] = True
        clean_bg = band & ~liquid
        if clean_bg.sum() < 500:
            continue
        flat = np.median(arr[clean_bg], axis=0)
        # anything that is neither background nor wave stays: the logo
        # Only real artwork survives, meaning the logo. A loose threshold here
        # keeps most of the old gradient instead and the flat fill never happens.
        strong = band & ~liquid & (np.abs(arr - flat).max(axis=2) > 80)
        # Bridge the gaps between letters first. The wordmark next to the logo is
        # thin strokes and each letter is its own small blob, so an area test
        # applied letter by letter drops the word and keeps only the ring.
        ...
        out[band] = flat
        out[keep] = arr[keep]
        print(f"  stripe {k + 1}: filled flat {tuple(int(v) for v in flat)}"
              + (f", {int(keep.sum())} px of artwork kept" if keep.sum() > 200 else ""))
    ...
    for i, comp in enumerate(comps):
        ...
        dy = ((ys_.min() + ys_.max()) * 0.5) - ((top.min() + bot.max()) * 0.5)
        cen = centre + dy

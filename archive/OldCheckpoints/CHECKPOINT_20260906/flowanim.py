...
def main():
    ...
    p.add_argument("--anchor-l", type=float, default=330)
    p.add_argument("--anchor-r-x", dest="anchor_r_x", type=float, default=1190)
    ...
    if args.base:
        ...
        if args.anchor_r > 0:
            lab_c, n_c = ndimage.label(mask > 0.5)
            yy, xx = np.mgrid[0:H, 0:W]
            rr = args.anchor_r * W / 1536.0
            for i in range(1, n_c + 1):
                ys_c, xs_c = np.where(lab_c == i)
                if len(ys_c) < 2000:
                    continue
                # Same height for both, matching how recolor_base.py draws them.
                cy = (ys_c.min() + ys_c.max()) / 2.0
                for cx0 in (args.anchor_l, args.anchor_r_x):
                    cx = cx0 * W / 1536.0
                    art |= np.hypot(xx - cx, yy - cy) < rr + 12
        clean = ~art

...
The generator keeps getting these wrong: above the figure instead of under it,
...
The right caption names the muscle group rather than a single muscle: the body
beside it is already saying which muscles, in colour, one at a time. A caption
that repeats the first badge is a word the row spends twice.

    python3 add_labels.py poster.jpeg -l base_push_layout.json \
        --labels 'BENCH PRESS|CHEST,PULL-UP|BACK,...' -o labelled.png
"""
import argparse
import json
import numpy as np
...
def main():
    ...
    p.add_argument("--labels", required=True,
                   help="five 'LIFT|MUSCLE GROUP' pairs, comma separated, top row first")
    args = p.parse_args()
    ...
    for (lift, group), row in zip(pairs, L["rows"]):
